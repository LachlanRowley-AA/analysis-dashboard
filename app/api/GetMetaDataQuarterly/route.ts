...
export async function GET(req: Request) {
  try {
    ...
    const increment = url.searchParams.get('increment') ?? '7'
    ...
    const first_params: QueryParams = {
      startDate: quarterStartDate.toISOString().split('T')[0],
      endDate: endDate.toISOString().split('T')[0],
      increment,
    }
    const data = await fetchFromMetaAPI(first_params)

    //Last quarter
    const lastQuarterEndDate = new Date(quarterStartDate)
    lastQuarterEndDate.setDate(lastQuarterEndDate.getDate() - 1)
    const last_params: QueryParams = {
      startDate: new Date(quarterStartDate.getFullYear(), quarterStartDate.getMonth() - 3, 1).toISOString().split('T')[0],
      endDate: lastQuarterEndDate.toISOString().split('T')[0],
      increment,
    }
